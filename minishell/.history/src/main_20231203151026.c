...
/*   Updated: 2023/12/03 15:10:26 by esilbor          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	do_builtins(char **cmd_tab, t_env **envb)
{
	...
	if (ft_strncmp(cmd_tab[0], "pwd", ) == 0)
		do_pwd(cmd_tab, envb);
	if (ft_strncmp(cmd_tab[0], "echo", ) == 0)
		do_echo(ft_tab_len(cmd_tab), cmd_tab);
	if (ft_strncmp(cmd_tab[0], "env", ) == 0)
		print_env(envb);
	if (ft_strncmp(cmd_tab[0], "export", ) == 0)
	{
		if (cmd_tab[1])
	...
	free(env_dup);
	if (ft_strncmp(cmd_tab[0], "unset", ) == 0)
		do_unset(cmd_tab, envb);
	if (ft_strncmp(cmd_tab[0], "cd", ) == 0)
		do_cd(cmd_tab, envb);
