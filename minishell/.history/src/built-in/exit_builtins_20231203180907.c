...
/*   Updated: 2023/12/03 18:09:07 by esilbor          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/minishell.h"

bool is_ll_overflow(const char *)
{
	int i;
	int sign;
	int	num_len;
	...
	sign = 1;
	num_len = 0;
	while ([i] == ' ' || ([i] >= '\t' && [i] <= '\r'))// Skipping leading white spaces
		i++;
	if ([i] == '-' || [i] == '+')// Checking sign
	{
		if ([i] == '-')
			sign = -1;
		i++;
	...
	int j = i;
	while ([j] >= '0' && [j] <= '9')// Calculate the length of the number part
	{
		j++;
		num_len++;
	}
	if (num_len > 19 || (num_len == 19 && sign != -1
		&& strcmp( + i, MAX_LL) > 0))// Length check
		return true; // Overflow
	if (num_len == 19 + 1 && sign == -1 && ft_strcmp( + i, MAX_LL) > 0
		&& leading_zeroes == 0)
