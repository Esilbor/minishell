...
/*   Updated: 2023/12/03 17:53:06 by esilbor          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool is_overflow(const char *nptr)
{
	int i = 0;
	int sign = 1;
	const char *max_str = "9223372036854775807"; // Max value for long long
	int max_str_len = 19; // Length of the max long long value as a string
	...
	// Length check
	if (num_len > max_str_len || (num_len == max_str_len && sign != -1 && strcmp(nptr + i, max_str) > 0))
		return true; // Overflow

	if (num_len == max_str_len + 1 && sign == -1 && strcmp(nptr + i, max_str) > 0 && leading_zeroes == 0)
