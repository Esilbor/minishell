...
/*   Updated: 2023/12/03 18:26:27 by esilbor          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/minishell.h"

static void	ll_overflow_parsing(const char *str, int *i, int *sign, int *zero_nb)
{
...
bool	is_ll_overflow(const char *str)
{
	...
	num_len = 0;
	zero_nb = 0;
	ll_overflow_parsing(str, &i, &sign)
	while (str[i + num_len] >= '0' && str[i + num_len] <= '9')
		num_len++;
	if ((num_len > 19) || (num_len == 19 && sign != -1 && ft_strcmp(str + i, MAX_LL) > 0))
		return true;
